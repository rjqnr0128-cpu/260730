...
st.caption("시군구 단위로 나이대별 인구 비율을 색칠해서 보여줍니다.")
...
def compute_sigungu_ratio(pop_df: pd.DataFrame, year: int, metric_key: str, age_start: int, age_end: int):
    """
    선택한 연도 + 선택한 나이 범위를 기준으로,
    시군구별 "해당 나이 비율(%)" 표를 만들어 돌려준다.
...
def compute_youth_auto_edges(pop_df: pd.DataFrame):
    """
    유소년 비율은 고령화율보다 훨씬 낮은 범위에 몰려 있어서, 같은 구간을 쓰면 지도가
    거의 한 색이 되어 버린다. 그래서 전체 연도의 데이터를 다 모아서, 실제 값을
    다섯 덩어리로 나누는 경계값을 자동으로 계산한다. (20% / 40% / 60% / 80% 분위수)
    """
    all_years = sorted(pop_df["연도"].unique())
    ratios = []
    for year in all_years:
        g = compute_sigungu_ratio(pop_df, year, "youth", 0, 14)
        ratios.extend(g["비율"].dropna().tolist())
...
max_year = int(population_df["연도"].max())
...
with control_col1:
    selected_year = st.slider("연도 선택", min_value=min_year, max_value=max_year, value=max_year, step=1)
...
# 시군구별 비율 계산 (선택한 연도 + 선택한 지표)
sigungu_df = compute_sigungu_ratio(
    population_df, selected_year, metric_info["key"], metric_info["age_start"], metric_info["age_end"]
)

# 구간 경계값 정하기: 고령화율은 고정값, 유소년 비율은 데이터 기반 자동 계산(연도와 무관하게 한 번만 계산)
if metric_info["fixed_edges"] is not None:
    bin_edges = metric_info["fixed_edges"]
    bin_labels = metric_info["fixed_labels"]
else:
    bin_edges, bin_labels = compute_youth_auto_edges(population_df)
...
# 6. 지도 그리기
# ---------------------------------------------------------
st.subheader(f"{selected_year}년 기준 시군구별 {metric_label}")
...
if len(mismatch_df) > 0:
    ...
    st.warning(
        f"⚠️ 다음 지역은 {selected_year}년 코드가 지도 경계 데이터와 맞지 않아 회색으로 표시했어요: {names}"
    )
else:
...
# 7. 지도 아래 순위 표
# ---------------------------------------------------------
st.subheader(f"{metric_label} 순위")
